...
> 状态：**第一步（MVP）+ 第二步（±10s combat log / 技能 CD / 状态胜率）+ 第三步（眼位·烟雾图层 / 多场索引 / lite 版）均已交付并发布公网**。
...
| 第三轮 | 「接着做」 | **第二步**：点英雄 → ±10s combat log 四 toggle + 技能 CD 三态（BKB/刷新球/TP）+ 状态胜率 | `3a961a4` |
...
**为什么不是"一个页面里切 970 场"**：完整版单场 4~6MB（±10s 明细是大头），970 场 ≈ 5GB，塞不进一个仓库/页面。所以拆三层：
...
| 完整 | `q7_replay_<mid>.html`（4~6 MB/场） | owner 关心的重点场次：含 ±10s **全量** combat log 明细 + 技能图标 |
...
- **窗口 = 当前播放时刻 ±10s**（owner 默认"以当前播放时刻为中心"），页面显示 `t−10 → t+10` 与四类条数。
...
| 3 | "只显示时间看不出来什么意思…阵亡英雄的头像、塔被摧毁放塔图标…时间轴放在地图上方、占满整页宽度" | **图标版**：击杀=阵亡英雄头像（26px 卡面居中裁切），塔/兵营/基地/肉山/侦查守卫=白描剪影按所属方染色；**环色=它属于哪一方**（绿天辉/红夜魇/灰无主·肉山），**上下位置=对谁有利**（两个维度分开表达）；mm:ss 从默认显示改为**关掉的开关**；两条泳道各自分四层错开，泳道高度按最高层算（原来会压到轴上的说明文字）；修掉肉山环色误落到夜魇红 | `1fc3013`、`6b07434` |
...
4. 泳道高度 = `4 + (NROW−1)×ROWH + MH`（NROW=4；ROWH 31/48、MH 30/42），
