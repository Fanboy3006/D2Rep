...
| 4 | "combat log 最好放在右边，占据另外半个屏幕宽度…我需要 combat log、地图和下面的英雄头像、还有时间轴都在一个屏幕之内" | ① **修掉真 bug**：`.left` 从来没闭合 → `.right`（明细/combat log）被塞进左栏里、右半屏全空（见下）；② 两栏改 `1fr / 1fr`（各占一半）；③ **整页锁一屏**：`body` 100vh flex 纵向排列、右栏自己滚、地图是唯一弹性元素（`fitCanvas()` 取 `min(可用宽, 可用高)` 作边长）；④ 时间轴层数按视口高自适应（2/3/4 层）、图例收成一行、±60s 小条并入控制行；⑤ 10 个头像从两行改成**一行**（天辉 5 ｜ 夜魇 5），省下的 57px 还给地图 | `38cfaa7` |
...
5. 高度预算（`.tmp/fit_budget.py` 估算，约值）：1920×1080 → 地图 **≈458px**、时间轴 267px；
   1600×900 → ≈408px；1366×768 → ≈250px；2560×1440 → ≈596px。
   只有"连 200px 地图都放不下"时才会退化成整页滚动（矮屏兜底，不裁切内容）。
6. **已知取舍**：地图是正方形、又被高度限制，所以在 1920 宽屏上它只占左栏（942px）中间一块，
   左右会留白。要换掉这个留白有三条路（owner 未拍板）：① 10 个头像改竖排放地图右侧（地图能到 ≈510px）；
   ② combat log 吃 60% 宽（超过半屏）；③ 给地图加个"最大化"开关（临时收起时间轴/顶部条）。
