...
| 3 | "只显示时间看不出来什么意思…阵亡英雄的头像、塔被摧毁放塔图标…时间轴放在地图上方、占满整页宽度" | **图标版**：击杀=阵亡英雄头像（30px 卡面居中裁切），塔/兵营/基地/肉山/侦查守卫=白描剪影按所属方染色；**环色=它属于哪一方**（绿天辉/红夜魇/灰无主·肉山），**上下位置=对谁有利**（两个维度分开表达）；mm:ss 从默认显示改为**关掉的开关**；两条泳道各自分三层错开，泳道高度按最高层算（原来会压到轴上的说明文字）；修掉肉山环色误落到夜魇红 | `1fc3013` |
...
4. 泳道高度 = `4 + (NROW−1)×ROWH + MH`（NROW=3；ROWH 27/44、MH 26/38），
   断言"泳道高度 ≥ 最高一层 + 26"防止图标压到轴上的说明。
5. **所有 60 个已发布 viewer（2 完整 + 58 lite）都重新生成了切片与页面**，
