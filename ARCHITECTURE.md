...
- **③ 世界范围**：权威 `WORLD_SPAN=18944`（=2×9472），来自 `default_ents.vents` 两个 `dota_minimap_boundary`（±9472,±9472）+ `worldspawn._dotatilegrid_fogbounds_*`。`map_annotations.py` 重写为权威 `NEUTRAL_CAMPS`(28，含档位+triggerName)、`ROSHAN`、并新增 `TWIN_GATES/FORTS/FOUNTAINS/SHOPS/TOWERS`。`export_match_viewer_dom.py` 的 Python 端 `WORLD` 与 JS 端 `var WORLD` 同步改为 `18944`。
- **④ 验证**：把 28 野点/22 塔/双生门/遗迹/Roshan/泉水画到真实底图上（`.tmp/overlay_auth.png`、`.tmp/overlay_payload.png`）肉眼复核——野点落丛林空地、塔沿三路、Roshan 在下路野区坑、双生门在对应河边路口，锚定一致；边角野点（如 `evil_10` @ -8576）为真实边缘野点。
- 遗留：`map_background.py` 内嵌的底图生成路径与是否同步 `WORLD_SPAN` 需与上文保持（若它自绘底图应改用同一 18944 口径）。
