...
**模块 B：单场 HTML 复盘（云玩家友好，v2 2026-09-04）** — `opendota_analysis/export_match_viewer.py <match_id|db> [--map 资产PNG] [--step N] [--no-icons]` → `dist/viewer_<id>.html`（单文件自包含，双击浏览器即可，无 Node/Python）：内嵌 底图 base64 + 10 英雄逐秒 `x/y/hp/hp_max/mana/mana_max` JSON + **官方英雄头像卡 base64**（128×72，源为官方 CDN `images/heroes/<名>_full.png`，个别英雄 404 时回退官网现行 `images/dota_react/heroes/<名>.png`；首次下载缓存至 `assets/hero_icons/`）。页面：Canvas 可**缩放/平移**大画布（滚轮+拖拽、按钮复位、点侧栏英雄聚焦）、时间滑块+线性插值、空格播放/暂停、**图标下实时血条+蓝条**（血量<30%变红；放大≥44px 时图标上方直接显示 `hp/hp_max ♥mana` 数字）、血蓝数值随动、英雄卡片带队色描边。时间轴以**位置数据起点归零**（顶部提示原始时间轴起点），避免录像前置空转段造成"前期不动"的观感。**原型 v2**：`dist/viewer_8822238357.html` 与 `dist/viewer_8942993144.html`（各约 2.8MB、10 英雄 2.7 万采样/场）。

**模块 B 批量导出** — `opendota_analysis/export_viewers_batch.py [--all|--limit N|--every N|--leagues a,b] [--step N] [--workers 4] [--skip-existing]` → `dist/viewers/viewer_<id>.html`。内部复用同一 `export()`（已重构供 CLI/批量共用；图标缓存写入原子化+加锁，多线程安全）。**试点（2026-09-04，均匀抽样 17 场跨 7 联赛）**：4 线程 6 秒完成、0 失败，平均 **3.03MB/场**。**全量（2026-09-04 实跑）**：970/970 ok、0 失败、421 秒、共 **2.94GB**（平均 3.09MB/场，含 10 头像卡 + 10 英雄约 2–5 万采样）；`--step 2/4` 可近乎线性压缩体积。图标已全量预取缓存：语料涉及的 **126/126 英雄** 官方头像卡（0 缺失，`assets/hero_icons/`，随 git 入库，离线/跨机可直接复用）。

**配套解析层富化与修复（2026-09-04）**：快照 extra 增加 `hp_max`/`mana`/`mana_max`（100% 可取，供血蓝条）；ward_destroyed 事件 properties 增加 `team`（供模块 A 配对）；**修复 hero 采样门控的 tick 奇偶相位饿死问题**（详见 §6.6 注记）后用 `opendota/parse_public.py --force` 全量重解析 970 场并复验：ok=970 fail=0，**970/970 位置轨迹 fill=100%**（此前"45 场 event_only"系误判，实为门控在全奇数相位下永不命中）；重解析后各分析模块（模块 A 事件数据不受影响，模块 B 位置数据受益）均基于新库。另修 `parse_public.py` 子进程读取编码（GBK 主机上 Rust 输出的 UTF-8 玩家名不再刷 UnicodeDecodeError）。
