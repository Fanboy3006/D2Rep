...
**模块 B（通用分享版，无 Canvas）** — `opendota_analysis/export_match_viewer_dom.py <match_id|db>` → `dist/viewer_<id>_lite.html`。与 v3 数据同源（含塔/野点/尾声折叠），但**不用 Canvas**：地图为 `<img>`、英雄/塔/野点/肉山为绝对定位 DOM 元素、时间轴为原生 `<input range>` + 插件式 JS，**初始局面（数据起点位置）已烘焙进 HTML**。因此：真浏览器、微信安卓内置 webview、iOS 系统预览(WKWebView) 都能渲染；即使部分环境禁用 JS 也会显示完整初始版图。**用于微信发给朋友直接看**（把 `viewer_<id>_lite.html` 作为文件发送即可，避免公网挂数据）。
