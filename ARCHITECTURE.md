...
- **第2步 批量 .dem 下载：部分完成并已通过端到端验证（2026-09-03 夜，F 盘主力机）** —— 工具：`opendota/list_matches.py`（按联赛名/ID 抓 match ID 表）、`opendota/batch_download.py`（流式 Range 断点续传 + 停滞看门狗/两次停滞跳过 + 状态 `.tmp/batch_download/<league>.json`）、`opendota/batch_summary.py`/`scan_dems.py`/`parse_public.py`/`verify_public.py`（统计/完整性/全量解析/三类校验）。**已下载 4 个联赛 514 场 .dem（约 88GB）**：19719 TI2026 正赛 147/147、19255 165/165、19696 185/185 全部完成，19101 17/102（中断点 44MB 已留）；**剩余 19101 尾部 + 19785/19422/19917 明日续传**（重跑同一条 batch_download 命令即续）。**验证结果**：魔数完整性 0 损坏；514/514 解析成功；`verify_public` 三分类 full=479 / event_only=35 / bad=0（共 924万快照 / 42.2万事件）；479 场跨场次热区 pooled 分析正常；事件聚合 purchase 30.6万、ward_placed 5.9万、ward_destroyed 5.7万（量级合理）。**新已知坑（§6.6）**：约 6.8% 的公开录像 .dem 没有 tick 流（无 CDemoTickTime），事件/身份照常但 **entity_snapshots 恒为空**（位置类分析不可用，购买/守卫等事件类不受影响）。解析入 catalog 待全部下载完成后进行（§8 第7步 intake 流程）
...
- **部分公开录像 .dem 没有 tick 流（2026-09-03 实测：514 场中 35 场 ≈6.8%）**：这类录像不含 CDemoTickTime 消息（`on_tick_start` 从不触发），解析仍正常产出 `player_identity` 与 `game_events`（购买/守卫实体 Created/Death 等都在），但 **`entity_snapshots` 恒为空**——位置热区等依赖快照的分析不可用，事件类（购买/视野等）不受影响。分布：19719=21、19255=11、19696=2、19101=1。校验工具 `verify_public.py` 将这类标为 event_only。若未来需要这些场次的位置数据，需改为非 tick 驱动的实体位置快照（订阅实体 origin 更新），属后续增强。
...
2. ~~跑通批量 .dem 下载脚本~~ **进行中（2026-09-03，执行机=F 盘主力机）**：单场工具已转正（`dota_parse/tools/fetch_replay_dem.py`：OpenDota 取 replay_url → 下载 → 魔数嗅探 bz2/zstd 解码 → 校验 PBDEMS2）。速度基线见进度速览（19543 valve.net ≈2.3–7.1MB/s；19719 国服域名 ≈0.1MB/s 只作兜底，但实测同文件可断点续传、网络波动时瞬时可达数 MB/s）。**批量实现（2026-09-03 新增）**：`opendota/batch_download.py --league <id>...`（流式 Range 断点续传 + 跨进程/关机续传 + `.dem.raw.part` 中间文件 + 状态 `.tmp/batch_download/<league>.json`，同命令复跑即续；`--max-downloads` 试跑、`--partial-test-bytes` 演练中断、`--log` 记录）；`opendota/list_matches.py --league-id|--league-name` 按联赛批量抓 match ID 表（TSV/CSV：match_id/时间/radiant_win/series/双方队id），供 match 管理用。下载完成 → 本地解析 → catalog 登记 `source='public'`（幂等，思路复用 `scheduler/intake_private.py`；分布/时段抽样建议覆盖不同阶段以反映真实波动）。**2026-09-03 夜实况**：TI2026 正赛+3 联赛共 514 场已下（约88GB），514/514 解析、479 场 full 校验通过，详见顶部进度速览；剩余 19101 尾+19785/19422/19917 重跑同命令续传即可
