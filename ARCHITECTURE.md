...
**模块 B：单场 HTML 复盘（云玩家友好，v3 2026-09-04）** — `opendota_analysis/export_match_viewer.py <match_id|db> [--map 资产PNG] [--step N] [--no-icons]` → `dist/viewer_<id>.html`（单文件自包含，双击浏览器即可，无 Node/Python）：内嵌 底图 base64 + 10 英雄逐秒 `x/y/hp/hp_max/mana/mana_max` JSON + **官方英雄头像卡 base64** + **野点/肉山标注**（`map_annotations.py`，源=devilesk/dota-interactive-map 的 7.33+ 实体坐标，与重放世界坐标逐塔对表一致；18 野点按档位着色▲、远古档◆金色、肉山坑圆圈）+ **每场 22 座塔的存活状态**（来自解析层 building 事件）。页面：Canvas **缩放/平移/双指捏合**（Pointer 事件，触屏可用）+ 图例开关（**野点 / 防御塔** 图层可单独显隐）+ 时间滑块+线性插值 + 空格播放/暂停 + 图标下实时血条蓝条（放大后图标上方显示 `hp/hp_max ♥mana`）+ 英雄卡片带队色描边 + **塔实时状态**（存活=队伍色圆点；已摧毁=灰底×）。**移动端适配**：viewport meta + `100dvh` + ≤860px 自动上下布局（地图在上、英雄列表在下横向滚动），手机不再空白。**尾声自动折叠**：对局结束后录像常继续空转到英雄满血静止（如 8822238357 在活动止点后多 7–14 分钟全静止），导出时按"最后走位/事件 +90s"裁剪并顶部注明"对局活动止于 mm:ss，静止段已折叠"（8822238357 raw 3441→有效 3009s）。**原型 v3**：`dist/viewer_8822238357.html`、`dist/viewer_8942993144.html`。
