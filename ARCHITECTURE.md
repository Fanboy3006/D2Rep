...
- **hero 位置采样曾受"tick 奇偶相位"静默饿死（2026-09-04 定位并修复，重大）**：Dota replay 文件的 tick 流存在整段只送**偶数 tick** 或只送**奇数 tick** 的相位（source2-demo 每次回调 `ctx.tick()` 大步长+2/-…推进，`tick % 30 == 0` 只在偶数相位命中）。原实现用 `tick % 30 == 0` 门控 1 秒采样，导致大量比赛只有零星采样窗口、甚至整场为空。实测单场（8822238357）：修复前每英雄 374 个采样点仅覆盖 [750,1122]+1s，修复后 2692 点连续覆盖 [750,3441]。**此前"45 场无 tick 流 / event_only"的结论是误判**——那 45 场其实是全奇数相位文件，`on_tick_start` 一直触发但门控永不命中；修复后 970/970 全部有数据。**修复方式**：改为"秒变化即采样"门控（记录上一次采样的 `tick/30`，变化才采样一次），与相位无关、天然 1 Hz 上限、同秒去重。修复后 `parse_public.py --force` 全量重解析 970 场（2026-09-04，ok=970 fail=0），校验：**970/970 每场 fill=100%（连续 1Hz）、单场样本 3–5 万行**。注意：此类文件的游戏内容前常有一段无实体内容的空转 tick（例如英雄在 ~750s 才出现），语义上 = 开局延迟/录像前置段，不是丢数据。
...
**共享地图底图（两个模块共用）** — `opendota_analysis/map_background.py` + 资产 `assets/dota_map_1024.png`。**该资产现在是官方当前版小地图纹理**：从本机 Dota2 安装 `maps/dota_737.vpk` 提取 `materials/overviews/dota_737`（`.vtex_c`），用 ValveResourceFormat 的 `Source2Viewer-CLI --decompile` 解成 1024×1024 PNG（官方 overview 同款，7.37 布局，7.41 游戏版本与此一致）。坐标映射口径：**官方底图四周有黑色 out-of-bounds 边，世界 (0,0) 不在图中心**，故用**泉水锚定标定**（各向同性最小二乘，两泉残差<1.3px）：`px=CALIB_OFFX+CALIB_K*x`、`py=CALIB_REF_Y−CALIB_K*y`，其中 `CALIB_K≈0.049038`、`CALIB_OFFX≈508.3019`、`CALIB_REF_Y≈504.5433`（详见 `map_annotations.py` 与 `MAP_PARSING.md`）。世界坐标与 replay 共用同一世界空间，塔/野点直接在此映射下对齐道路。**数值校验**（用真实重放数据）：泉水/角落簇像素 radiant≈(144–172,846–853) ↔ dire≈(879,160–180) 基本对称；全场像素落在 [52..960] 与地图留边吻合。肉眼复核图：`.tmp/overlay_full.png`（21.6 万点叠加）、`.tmp/ward_survival_heat.png`、`.tmp/preview_vectors.png`。

**模块 A：假眼存活/被反热力图** — `opendota_analysis/ward_survival_heatmap.py`。数据依赖解析器富化（见下）：ward_destroyed 带 `properties.team`（combat-log target_team）。按 (match, team) FIFO 配对 placed/destroyed observer 得存活时长，筛 ≥300s，按坐标 400 单位分 bin，在底图上以红色深浅表示"被反占比"输出 PNG+文字摘要。**全量结果（970 场，2026-09-04 重解析后复跑确认稳定）**：≥300s 存活样本 20035（被反 2809=14.0%、自然过期 17226=86.0%；被反数与修复前一致，过期数因事件配对口径微升 259）；过期寿命中位 368s（p95≈425s）→ **假眼上限约 6 分钟，≥300s 判定成立**；高被反点位集中在暴露的线侧/野区入口（如天辉下路外侧野区、夜魇上路外侧野区等，区域名为粗略方位描述）。跑法：`python opendota_analysis/ward_survival_heatmap.py`。

**模块 A2：交互式热力图探索（HTML）** — `opendota_analysis/ward_heatmap_explorer.py` → `dist/ward_heatmap_explorer.html`（单文件，双击即用，约 2.3MB）：内嵌全部 970 场的 38,813 条假眼实例 `[x,y,存活时长,被反0/1]`（配对口径与模块 A 完全一致，含游戏结束截尾按过期计）+ 底图 base64。页面上：**"显示热力"开关** + **两个存活时长滑块（下限/上限）**——只统计落在 `[lo,hi]` 区间内的样本，格子"被反占比"热力与文字摘要（区间样本数、被反数/占比、自然过期数、n≥5 高发格 Top6）在拖动时即时重算；另有"样本点"图层可叠加查看密度。默认区间 300–600s 即复现模块 A 口径。

**模块 B：单场 HTML 复盘（云玩家友好，v3 2026-09-04）** — `opendota_analysis/export_match_viewer.py <match_id|db> [--map 资产PNG] [--step N] [--no-icons]` → `dist/viewer_<id>.html`（单文件自包含，双击浏览器即可，无 Node/Python）：内嵌 底图 base64 + 10 英雄逐秒 `x/y/hp/hp_max/mana/mana_max` JSON + **官方英雄头像卡 base64** + **野点/肉山标注**（`map_annotations.py`，**源=游戏真实地图实体**：从 `maps/dota_737.vpk` 的 `maps/dota/entities/default_ents.vents`（编译的默认实体 keyvalues，ValveResourceFormat 解出）解析 `npc_dota_neutral_spawner` 的 `origin` 世界坐标，28 野点按刷怪点取精确位置、档位着色▲、远古档◆金色、肉山坑圆圈；同一文件还提供双生门(`npc_dota_unit_twin_gate`)、遗迹/远古(`npc_dota_fort`)、泉水、商店、眼位(`ward_spot_*`)、神符、岗哨塔等权威坐标）+ **每场 22 座塔的存活状态**（来自解析层 building 事件）。页面：Canvas **缩放/平移/双指捏合**（Pointer 事件，触屏可用）+ 图例开关（**野点 / 防御塔** 图层可单独显隐）+ 时间滑块+线性插值 + 空格播放/暂停 + 图标下实时血条蓝条（放大后图标上方显示 `hp/hp_max ♥mana`）+ 英雄卡片带队色描边 + **塔实时状态**（存活=队伍色圆点；已摧毁=灰底×）。**移动端适配**：viewport meta + `100dvh` + ≤860px 自动上下布局（地图在上、英雄列表在下横向滚动），手机不再空白。**尾声自动折叠**：对局结束后录像常继续空转到英雄满血静止（如 8822238357 在活动止点后多 7–14 分钟全静止），导出时按"最后走位/事件 +90s"裁剪并顶部注明"对局活动止于 mm:ss，静止段已折叠"（8822238357 raw 3441→有效 3009s）。**原型 v3**：`dist/viewer_8822238357.html`、`dist/viewer_8942993144.html`。

**技能 CD 追踪（2026-09-04 新增，解析层 + viewer）**：解析层 `AbilityExtractor`（parse.rs）按整秒从英雄实体 `m_vecAbilities.*` 的技能句柄逐个读技能 `m_fCooldown`（冷却剩余秒，float），进/出 CD 时刻各写 1 条 `ability_cd_start` / `ability_cd_end` 事件（含 hero/ability/remaining）。**实测这一场（8822238357）**：10 英雄全覆盖、1189 起/1189 止（配对成 1189 段冷却区间）、57 个技能（含 Doom 吞噬的野怪技能）。显示名由技能类名去掉 `CDOTA_Ability_` 与英雄前缀推断（英文）。lite viewer 底部新增"**技能CD**"面板：每英雄一排技能钮，**冷却中灰显并显示剩余秒（取整）**，随时间轴实时更新。图标暂未做（官网无统一公开技能图标 CDN，需外部资源）。字段坑：不是 `m_flCooldownRemaining`（读不到），正解是 `m_fCooldown`；技能→英雄归属靠英雄实体上的 `m_vecAbilities` 句柄（技能实体自身无 owner 字段）。

**模块 B（通用分享版，无 Canvas）** — `opendota_analysis/export_match_viewer_dom.py <match_id|db>` → `dist/viewer_<id>_lite.html`。与 v3 数据同源（含塔/野点/尾声折叠），但**不用 Canvas**：地图为 `<img>`、英雄/塔/野点/肉山为绝对定位 DOM 元素、时间轴为原生 `<input range>` + 插件式 JS，**初始局面（数据起点位置）已烘焙进 HTML**。因此：真浏览器、微信安卓内置 webview、iOS 系统预览(WKWebView) 都能渲染；即使部分环境禁用 JS 也会显示完整初始版图。**公网分享（2026-09-04 采用并验证）**：把 `<match_id>_lite.html` 推到公开仓库 `Fanboy3006/d2rep-share` 并启用 GitHub Pages，得到 `https://fanboy3006.github.io/d2rep-share/viewer_<id>_lite.html`——**实测朋友安卓经该链接可正常浏览**（发 .html 文件会走系统预览/受限，失败；公网链接成功）。技能图标来自 Steam CDN `apps/dota2/images/abilities/<key>_lg.png`（key=类名 snake_case，缓存 `assets/ability_icons/`）；官方选手ID来自 OpenDota `/matches/<id>` 的 `players[].name`（按 account_id=steam64-76561197960265728 匹配，生成时联网取一次，缓存 `.tmp/op_match_<id>.json`），玩家名超链接到其 OpenDota 主页。**注意**：公网链接对持有 URL 者公开该场数据。

**模块 B 批量导出** — `opendota_analysis/export_viewers_batch.py [--all|--limit N|--every N|--leagues a,b] [--step N] [--workers 4] [--skip-existing]` → `dist/viewers/viewer_<id>.html`。内部复用同一 `export()`（已重构供 CLI/批量共用；图标缓存写入原子化+加锁，多线程安全）。**试点（2026-09-04，均匀抽样 17 场跨 7 联赛）**：4 线程 6 秒完成、0 失败，平均 **3.03MB/场**。**全量（2026-09-04 实跑）**：970/970 ok、0 失败、421 秒、共 **2.94GB**（平均 3.09MB/场，含 10 头像卡 + 10 英雄约 2–5 万采样）；`--step 2/4` 可近乎线性压缩体积。图标已全量预取缓存：语料涉及的 **126/126 英雄** 官方头像卡（0 缺失，`assets/hero_icons/`，随 git 入库，离线/跨机可直接复用）。**注意**：重解析新增塔/兵营事件后，如需让 dist/viewers 旧文件包含塔图层，需按需重新批量导出（未经用户同意不擅自跑大批量）。

**配套解析层富化与修复（2026-09-04）**：快照 extra 增加 `hp_max`/`mana`/`mana_max`（100% 可取，供血蓝条）；ward_destroyed 事件 properties 增加 `team`（供模块 A 配对）；**修复 hero 采样门控的 tick 奇偶相位饿死问题**（详见 §6.6 注记）后用 `opendota/parse_public.py --force` 全量重解析 970 场并复验：ok=970 fail=0，**970/970 位置轨迹 fill=100%**（此前"45 场 event_only"系误判，实为门控在全奇数相位下永不命中）；另修 `parse_public.py` 子进程读取编码（GBK 主机上 Rust 输出的 UTF-8 玩家名不再刷 UnicodeDecodeError）。**防御塔/兵营事件（v3 新增）**：解析层 `BuildingExtractor`（§6.6/parse.rs）按与英雄相同的"整秒门控"跟踪 `CDOTA_BaseNPC_Tower`（双方各 11 座）、`CDOTA_BaseNPC_Barracks`（各 6）、`CDOTA_BaseNPC_Watch_Tower`，以 `entity.index()` 区分同类的多实例；首见写 `building_spawn`（world x/y + team），血量归零或实体连续 2 秒消失写 `building_destroyed`（时刻=最后可见秒+1）。**第二次全量重解析（2026-09-04，460s，ok=970）**：校验 **970/970 每场 22 座塔 spawn、摧毁 8–19 座**；event-only 均已包含。塔坐标为静态（不随对局移动），viewer 端以其 spawn 坐标 + destroyed 时刻逐帧判定存活。

**地图数据链：底图 + 野点/入口/眼位全部出自游戏真实地图（2026-09-05 重构）** — 为解决此前野点/入口/眼位与底图"对不上"的根因（旧野点来自第三方 devilesk/mapdata 静态数据、与官方 overview 图不一致），整条链改为**单一权威来源 = 游戏内当前版地图**：

- **① 权威底图**：从本机安装 `maps/dota_737.vpk` 用 `Source2Viewer-CLI -i <path> --vpk_filepath materials/overviews/dota_737 --decompile` 解出官方 overview 纹理（`.vtex_c`→1024×1024 PNG），写入 `assets/dota_map_1024.png`（字节级取自游戏，非社区镜像）。清理脚本 `.tmp/save_map737.py`。
- **② 权威坐标**：同一 `maps/dota_737.vpk` 解出 `maps/dota/entities/default_ents.vents`（VRF `--vpk_filepath maps/dota/entities/default_ents.vents_c --decompile`，4.1MB 实体 keyvalues），正则切 `====N====` 块取 `classname`+`origin`。关键类：`npc_dota_neutral_spawner`(28 野点)、`npc_dota_tower`(22 塔)、`npc_dota_unit_twin_gate`(2 双生门)、`npc_dota_fort`(2 遗迹)、`ent_dota_fountain`(2 泉水)、`npc_dota_roshan_spawner`(肉山)、`ward_spot_*`(17 眼位)、`dota_item_rune_spawner_*`(神符)、`ent_dota_shop`/`trigger_shop`(商店)、`npc_dota_watch_tower`(4 岗哨)。
- **③ 世界→像素标定（修复塔/野点偏路）**：不假设世界原点=图中心、也不假设 ±9472 铺满整图。官方底图四周有黑色 out-of-bounds 边，playable 内容区仅中央子框（实测像素 x[46..980] y[44..956]），且世界(0,0)映射到像素 (508.30,504.54) 而非 (512,512)。用两泉（天辉世界(−7456,−6938)→px(141.5,843.5)、夜魇世界(7408,6848)→px(872.75,170)，肉眼读放大图）做各向同性最小二乘，得 `CALIB_K≈0.049038`、`CALIB_OFFX≈508.3019`、`CALIB_REF_Y≈504.5433`（残差<1.3px）。`map_annotations.py` 提供 `world_to_px()`/`CALIB_*`，两个 viewer（Python `pct` + JS `pct`/`w2s`）与 `map_background.map_to_px` 全部改用它。先前的 `WORLD_SPAN=18944` 口径（=2×9472）是 playable 边界而非整图跨度，已废弃；整图跨度实际为 `1024/K≈20881.8`。
- **④ 验证**：把 28 野点/22 塔/双生门/遗迹/Roshan/泉水画到真实底图上（`.tmp/overlay_auth.png`、`.tmp/overlay_payload.png`、`.tmp/tower_road_check.png`）肉眼复核——野点落丛林空地、塔沿三路、Roshan 在下路野区坑、双生门在对应河边路口，锚定一致；边角野点（如 `evil_10` @ -8576）为真实边缘野点。
- **⑤ 矢量地图图标（2026-09-05 新增）**：游戏内塔/野点/眼位/Roshan 小地图符号是程序化矢量绘制、**无独立 PNG**，故自绘一套语义清晰、可上色、可缩放的 SVG 图标（`opendota_analysis/map_icons.py`）：`svg_tower`(绿/红尖塔，摧毁变灰)、`svg_camp`(0..2 怪剪影、3 远古金皇冠)、`svg_ward`(蓝圆眼/蓝菱形真眼)、`svg_roshan`、`svg_ancient`、`svg_fountain`、`svg_gate`；以 `data:image/svg+xml;base64` 内嵌到 `<img>`，移动端 webview 也兼容。DOM viewer 的塔/野点/Roshan/眼位均由 SVG 渲染。
- **⑥ 可重复地图解析流程**：`MAP_PARSING.md` 记录每次 Dota 大版本更新重新解码底图、解析 `default_ents.vents`、重标 `CALIB_*`、验证到 git 的完整清单。
- 遗留：底图用官方 overview 1024×1024（原生分辨率）；"清楚到每一棵树"需从 3D world 网格渲染（`world.vwrld_c`/`terrain_previews/*.vpk`），正交投影与 minimap 姿态未必一致，本线暂未做。
