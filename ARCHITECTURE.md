...
- **第2步 批量 .dem 下载：进行中（2026-09-03 起，F 盘主力机）** —— 工具已落地并启动：`opendota/list_matches.py`（按联赛名/ID 批量抓 match ID 表格）+ `opendota/batch_download.py`（**文件级 Range 断点续传**：写 `.dem.raw.part`，断线/重启后同命令续传；状态 `.tmp/batch_download/<league>.json`，日志 `--log`）。首批范围：**TI2026 正赛 19719**（147 场，优先）＋六个目标联赛（19255/19696/19101/19785/19422/19917，用户给出的 6 场 match 所属联赛；逐场探测，无 replay_url 记 unavailable）。产物 `dems/public/<league>/<id>.dem`。TI 区预选赛（19891-94）明确不做。已实测：8942993144（国服域名）3MB 模拟中断 → Range 续传 → zstd 解码 147MB .dem 完成。解析入 catalog 待下载完成后进行（§8 第7步 intake 流程）
