...
**圆内一道内环 = 状态**，右上角一个 **TP 徽标**（用 hero_ultimates.py 的相关逻辑见 §6.1）。
地图下方工具条新增 **大招 / TP / 队伍** 三个按钮切换内环含义：

| 模式 | 内环 | 右上 TP 徽标 |
|---|---|---|
| **大招**（默认） | 绿 = 就绪 ｜ 灰 = 冷却中 ｜ 深灰 = 该时刻没有数据 | 图标照常 |
| **TP** | 蓝 = 可用 ｜ 橙 = 冷却中 | 图标照常 |
| **队伍** | 不画内环 | 图标照常 |

**TP 徽标（owner 第二轮细化：不要色块小点，要用 TP 图标 + TP 冷却）**：徽标画的是**回城卷轴的
道具图标**（`opendota_analysis/assets/item_icons/tpscroll.png`，随载荷内嵌），尺寸 24px（原来那个
小圆点只有 9px）：**可用 = 图标正常 + 蓝边**；**冷却中 = 图标压暗 + 灰边，并在右下角挂一个琥珀色
秒数小牌**。这样"这是 TP"和"还差几秒"同时看得见。地图上的圆点用同一套画法（`drawTpBadge`，尺寸随
半径缩放 ≈ 1.45R），CD 面板里那个 TP 格子也从文字 "TP" 换成了这张图标。
★ 徽标与秒数牌必须放在头像框**内部**：`.hero` 是 `overflow:hidden`（用来裁圆角头像），
第一版把秒数牌挂在框外右下角，数字被裁掉了一半。
