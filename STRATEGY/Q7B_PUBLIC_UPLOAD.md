...
**唯一的数据缺口**：技能 CD 三态面板依赖 `dems/db/`（Q5 版老库）的 `ability_cd_*` / `item_cd_*` 事件，
而**当前 parser 已经不再产出这些事件**（提取器还在源码里但是死代码，见 §2.3）。
个人录像没有老库 → **CD 面板会是空的**（页面会如实显示"不提供"，不硬造）。其余全部可用。
...
**两条补法（二选一，需 owner 拍板）**：
1. **重新接线**（推荐，若 CD 面板对 Q7B 重要）：把 `AbilityExtractor` 在实体遍历里实例化、收集事件、
   调 `build_ability_event_rows()` 写进 `game_events`。代码在，但已停用一段时间，
   **需要重新验证**（预计 0.5~1 天，含单场对账 + 与老库逐事件比对）。
2. **不补**：Q7B 明确标注"技能 CD 需要老 parser 的实体事件，本版不提供"；
   或退回"combat_log 的 ability 施放事件 + 冷却常量表"（就是当初 owner 想避开的常量表方案）。
